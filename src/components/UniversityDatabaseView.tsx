...
import { MajorCategory, University, ExchangeRateData } from '../types';
import { 
  addToWishlist, 
...
export const UniversityDatabaseView: React.FC = () => {
  ...
  const filteredUnis = HK_UNIVERSITIES.filter((uni) => {
    ...
    const matchesSearch = 
      ...
      uni.majors.some(m => m.nameZh.includes(searchTerm) || m.nameEn.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="space-y-8 animate-fadeIn max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold font-serif text-slate-900 mb-1">
              香港高校全景数据库 (Hong Kong Universities Index)
            </h2>
            <p className="text-slate-500 text-xs sm:text-sm max-w-3xl leading-relaxed">
              收录香港公立八大与优质私立高校的最新 QS 排名、官方硕士项目、招录均分基准、学费区间与英语六级/雅思接受政策。
            </p>
          </div>

          {/* 实时学费计价币种切换 */}
          <div className="flex items-center space-x-1.5 bg-slate-100 p-1.5 rounded-xl self-start md:self-auto border border-slate-200">
            <span className="text-xs font-bold text-slate-500 px-2">学费币种：</span>
            <button
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t border-slate-100">
          ...
          <div className="flex items-center">
            <label className="flex items-center space-x-2 cursor-pointer text-xs sm:text-sm font-semibold text-slate-700 select-none">
              <input
                type="checkbox"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredUnis.map((uni) => (
          <div 
            key={uni.id}
            className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-sm hover:shadow-lg transition-all space-y-4 flex flex-col justify-between"
          >
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2.5">
                  {uni.majors.map(m => {
                    const tuitionDisplay = displayCurrency === 'CNY' && ratesData
                      ? convertHKDToCurrency(m.tuitionHKD, 'CNY', ratesData).formatted
                      : m.tuitionHKD;

                    return (
                      <div key={m.id} className="p-3 rounded-xl border border-slate-100 bg-slate-50/60 hover:bg-slate-50 transition-colors shadow-2xs space-y-2 text-xs">
                        ...
                        <div className="pt-1.5 flex items-center justify-between gap-2 border-t border-slate-100">
                          <button
                            onClick={() => {
                              addToWishlist({
                                ...
                                majorId: m.id,
                                majorNameZh: m.nameZh,
                                majorNameEn: m.nameEn,
                                faculty: m.faculty,
                                tuitionHKD: m.tuitionHKD,
                                status: 'planning'
                              });
                              setSavedMajorId(m.id);
                              setTimeout(() => setSavedMajorId(null), 2000);
                            }}
                            className={`inline-flex items-center space-x-1 px-2.5 py-1 rounded-lg text-[11px] font-bold border transition-colors cursor-pointer ${
                              savedMajorId === m.id
                                ? 'bg-emerald-50 text-emerald-800 border-emerald-300'
                                : 'bg-white hover:bg-amber-50 text-slate-700 hover:text-amber-900 border-slate-200 shadow-2xs'
                            }`}
                          >
                            <Bookmark className={`w-3 h-3 ${savedMajorId === m.id ? 'fill-emerald-600 text-emerald-600' : 'text-slate-400'}`} />
                            <span>{savedMajorId === m.id ? '已收藏 ✓' : '加入愿望单'}</span>
                          </button>
                          ...
                          {m.officialUrl && (
                            <a
                              ...
                              className="inline-flex items-center space-x-1 text-[11px] font-bold text-amber-800 hover:text-amber-950 bg-white hover:bg-amber-50 px-2.5 py-1 rounded-lg border border-slate-200 shadow-2xs transition-colors"
                            >
                              <span>🔗 官方网申</span>
            ...
            {/* 招录贴士 */}
            <div className="text-xs text-slate-500 pt-3 border-t border-slate-100 leading-relaxed">
